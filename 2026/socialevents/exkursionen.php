<!doctype html>
<html lang="de">

<head>
	<?php include "../inc/meta.inc"; ?>

	<base href="<?php echo dirname($_SERVER['SCRIPT_NAME']); ?>">
	<title>FOSSGIS 2026 - Exkursionen im Rahmenprogramm</title>

	<link rel="stylesheet" href="./css/bootstrap.css">
	<link rel="stylesheet" href="./css/normalize.css">
	<link rel="stylesheet" href="./css/base.css">
	<link rel="stylesheet" href="./css/expo.css">
	<link rel="stylesheet" href="./css/print.css" media="print">
	<link rel="stylesheet" href="./fontawesome/css/all.css">

</head>

<body id="expo">

	<p><a href="/2026/socialevents">Zurück zur Startseite</a></p>

	<section class="section">
		<div class="container">
			
		<div class="row">
		<div class="col-lg-9 mb-4 mb-lg-0 mt-3">
		<h3 id="Exkursion" name="Exkursion">Exkursionsangebote im Rahmen der FOSSGIS-Konferenz 2026</h3>
		<p>hier werden die Exkursionen, die im Rahmenprogramm der FOSSGIS 2026 angeboten werden, ausführlich beschrieben.</p>
		<p>Eine Anmeldung ist im Rahmen der Ticketbuchung zur Konferenz notwendig. </p>
		<p>Falls Sie schon ein Konferenzticket gebucht haben und zusätzlich eine Exkursion oder Workshop hinzubuchen wollen, schreiben Sie eine <a href="mailto:konferenz-orga@fossgis.de? subject=Nachbuchung_Zusatzprodukt"> E-Mail</a> an das Konferenzteam und bitten um Nachbuchung unter Angabe Ihres Namens, E-Mail-Adresse, Nummer der Konferenzticketbestellung, was genau hingebucht werden soll (Titel + Datum). </p>

		<h4 id="orangerStreifen" name="orangerStreifen" class="highlight"></h4>
			
		<h4 id="ExkursionKartensammlung" name="ExkursionKartensammlung">Exkursion - Besondere Stücke der Kartensammlung</h4>
		<ul>
		<li>Termine:
			<ul><li>Dienstag, 24.03.2026 16 Uhr</li>
			<li>Freitag, 27.03.2026, 17 Uhr</li></ul>
		</li>
		<li>Treffpunkt/ Ort:  Historisches Gebäude, Papendiek 14, Haupteingang, Koordinate: <a href="https://openstreetmap.de/karte/?zoom=19&lat=51.53392&lon=9.93207&layers=B00TT">51.53392, 9.93207</a></li>
		<li>Betreuung: Malte Semmler 
		<li>Dauer: 90 Minuten</li>
		<li>Anmeldung erforderlich, Schutzgebühr: 5 EUR</li>
		<li>Die Kartensammlung sowie besondere Stücke der Kartensammlung werden gezeigt.</li></ul>

		
		<h4 id="orangerStreifen" name="orangerStreifen" class="highlight"></h4>
				
		<h4 id="ExkursionForumWissen" name="ExkursionForumWissen">Führung durch das Forum Wissen</h4>
		<p>Es werden samstags um 15 Uhr und sonntags um 11 Uhr öffentliche Führungen in deutscher Sprache angeboten. Die Führungen dauern 60 bis 90 Minuten. Weitere Infos siehe <a href="https://www.forum-wissen.de">Webseite</a>  Forum Wissen.</p>

		<ul>
		<li>Termine:
			<ul><li>Mittwoch, 25.03.2026, 15 Uhr</li>
			<li>Donnerstag, 26.03.2026, 15 Uhr</li></ul>
		<li>Treffpunkt: Forum Wissen, Berliner Straße 28, 37073 Göttingen (Nähe Bahnhof)</li>
		<li>Betreuung: Frau Sohnrey</li>
		<li>Dauer: 90 Minuten</li>
		<li>Anmeldung erforderlich, Schutzgebühr: 5 EUR</li>
		<li>Führung durch das Wissensmuseum der Universität Göttingen. Das [Forum Wissen](https://www.forum-wissen.de) ist ein Ort der Begegnung und es geht um die Frage, wie Wissen entsteht.</li>
		</ul>

	
		<h4 id="orangerStreifen" name="orangerStreifen" class="highlight"></h4>


	
	<h4 id="ExkursionErdbebenwarte" name="ExkursionErdbebenwarte">Besuch der Wiechert'schen Erdbebenwarte Göttingen</h4>
	<ul>
	<li>Termin: Dienstag, 24.03.2026, 16 Uhr</li>
	<li>Treffpunkt/ Ort:  Wiechert’schen Erdbebenwarte Göttingen (<a href="https://www.erdbebenwarte.de/anfahrt/">siehe Anfahrtsbeschreibung</a>)</li>
	<li>Betreuung: Udo Wedeken 
	<li>Dauer: 90 Minuten</li>
	<li>Anmeldung erforderlich, Gebühr: 10 EUR</li>
	<li>Besuch der ältesten, heute noch mit wissenschaftlich brauchbaren Seismographen laufende Erdbebenwarte der Welt. Die Wiechert’sche Erdbebenwarte Göttingen ist eine voll funktionsfähige seismologsiche Messstation und ein begehbares Technikmuseum zugleich. Besucher sehen hier historische und moderne Seismographen oder wie einst mit dem Passage-Instrument die astronomische Zeit bestimmt wurde.</li></ul>

		<h4 id="orangerStreifen" name="orangerStreifen" class="highlight"></h4>
	

	<h4 id="ExkursionRechenzentrum" name="ExkursionRechenzentrum">Exkursion - Führung durch das Rechenzentrum der GWDG</h4>

	<ul>
	<li>Termine:
			<ul><li>Mittwoch, 25.03.2026, 12:30 Uhr</li>
			<li>Mittwoch, 25.03.2026, 13:15 Uhr</li></ul>
	<li>Treffpunkt: Rechenzentrum der GWDG (RZGö) (vom ZHG mittels der Buslinien 21,22,23 innerhalb von ca. 10 Minuten erreichbar)</li>
	<li>Betreuung: Dr.-Ing. Patrick Höhn</li>
	<li>Dauer: 45-60 Minuten</li>
	<li>Anmeldung erforderlich, <a href="https://files.fossgis.de/Konferenz/2026/Exkursion_Rechenzentrum_Zutrittsregeln.pdf">Zutritts- und Verhaltensregeln</a> sind zu beachten.</li>
	<li> Die Arbeitsgruppe "Computing" bietet eine Führung durch das Hochleistungsrechenzentrum in Göttingen. Es gibt Ihnen einen kurzen Überblick die Dienstleistungen der GWDG sowie über die technische Ausstattung, mit der wir HPC- und IT-Dienstleistungen für Unternehmen und Hochschulen in Deutschland erbringen.</li>
	</ul>

		<h4 id="orangerStreifen" name="orangerStreifen" class="highlight"></h4>

	<h4 id="ArchäologischeStadtführung" name="ArchäologischeStadtführung">Archäologische Stadtführungen</h4>

	<ul>
	<li>Titel: Archäologische Stadtführung - Schwerpunkt Stadtbefestigung</li>
	<li>Termine:
			<ul><li>Mittwoch, 25.03.2026, 15:30 Uhr</li>
			<li>Donnerstag, 26.03.2026, 15:30 Uhr</li></ul>
	<li>Treffpunkt: Düstere Straße 21</li>
	<li>Betreuung: Jannik Bönsch</li>
	<li>Dauer: 90 Minuten</li>
	<li>Anmeldung erforderlich, Schutzgebühr: 5 EUR</li>
	<li>Die Gruppe wird zweigeteilt, die eine Hälfte wird in der Düsteren Straße 21 archäologische Funde aus Göttingen kennenlernen und die andere einen kurzen Rundgang durch die Stadt machen und die Befestigungsanlagen und Fundstellen kennenlernen. Nach 45 Minuten werden die beiden Gruppen tauschen.</li>
	
		<h4 id="orangerStreifen" name="orangerStreifen" class="highlight"></h4>
	</ul>
	<ul>
	<li>Titel: Archäologische Stadtführung - Schwerpunkt Vom Dorf zur Stadt</li>
	<li>Termin: Freitag, 27.03.2026, 15:30 Uhr</li>
	<li>Treffpunkt: Düstere Straße 21</li>
	<li>Betreuung: Frank Wedekind</li>
	<li>Dauer: 90 Minuten</li>
	<li>Anmeldung erforderlich, Schutzgebühr: 5 EUR</li>
	<li>In einem 1, 5 stündigen Rundgang durch die Stadt werden die wichtigen Orte begangen, die Göttingen vom Dorf zur Stadt werden lassen. Angefangen bei dem „Alten Dorf“ und der Albani Kirche über die spätere Stadtgründung mit dem Markt und den drei „neuen“ Pfarrbezirken in der heutigen Stadtmitte.</li>
	</ul>

		<h4 id="orangerStreifen" name="orangerStreifen" class="highlight"></h4>

	

	<h4 id="physikCabinett" name="physikCabinett">Exkursion - Physicalisches Cabinett</h4>

	<ul>
	<li>Termin: Samstag, 28.03.2026, 10:30 Uhr</li>
	<li>Treffpunkt: Eingang <a href="https://www.uni-goettingen.de/de/47114.html">Physicalisches Cabinett</a></li>
	<li>Betreuung: Daniel Steil</li>
	<li>Dauer: 90 Minuten</li>
	<li>Anmeldung erforderlich, Schutzgebühr: 5 EUR</li>
	<li>Die Sammlung historischer physikalischer Instrumente am I. Physikalischen Institut, kurz Physicalisches Cabinet, erlaubt ihren Besuchern anhand von historischen Messapparaturen einen Einblick in die Geschichte der Physik an der Georg-August-Universität Göttingen.</li>
	</ul>

	<p><a href="/2026/socialevents">Zurück zur Startseite</a></p>
		
			</div>
			<div class="col-lg-3">
				<div class="mb-4">

						<h6 class="mb-0">Kontakt für Fragen<br>
							
							E-Mail: <br>
							<a href="mailto:konferenz-orga@fossgis.de">konferenz-orga@fossgis.de</a><br></h6>
					</div>

				</div>	

			</div>
		</div>

	</section>

	<?php include('../inc/footer.inc'); ?>
</body>
